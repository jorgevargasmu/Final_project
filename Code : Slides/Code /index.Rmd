...
data <- read.csv(here("data", "Behavioral_nat.csv"))
```
...
write.csv(data_tidy, file = here("clean_data", "data_tidy.csv"))
```
...
##ACC: Logistic regression with glmer
...
model_acc_AB <- glmer(Accuracy ~ Session * Condition + (1 + Session * Condition | ID) + (1 | Item), 
  data = data_tidy, 
...
#saveRDS(model_acc_AB, "models") #write path
```
